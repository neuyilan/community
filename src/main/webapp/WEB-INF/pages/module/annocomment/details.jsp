...
<script type="text/javascript">
        ...
        //删除评论
        function deleteComment(commentId, annoId) {
        	var bool = window.confirm("您确定要删除该条评论吗？");
    	    if(bool) {
    	    	$.getJSON('${ctx}/business/businessAnnoComment/delete.do', {id :commentId, annoId:annoId }, function(data) {
                    alert(data.message);
        ...
        function saveComment(commentId) {
        	...
        	if(comment == ''){
        		...
        		$.post('${ctx}/business/businessAnnoComment/replySave.do', {
            		commentId: commentId,
            		annoId: $('#annoId').val(),
      ...
      //获取多参数
    	function getParams() {
    		...
    		var params = {annoId: '${businessAnno.annoId}', keyWord: keyWord};
    		return params;
    	}
        ...
        function jump(pageNo, pageSize) {
        	...
        	$.ajax({
        		type: 'post',
                url: '<%=path %>/business/businessAnnoComment/getPageList.do',
                dataType: 'json',
                data: params,
                cache: false,
                success: function (data) {
                    ...
                    if(rows.length > 0) {
                    	...
                    	for(var i=0;i<rows.length;i++) {
                        	...
                        	var htmlDom = ''
                        		+ '<div id="comment1_'+row.commentId+'" class="cominfo">'
                        		...
                        		+ '<span class="del1" title="删除该评论" onclick="deleteComment('+row.commentId+', '+row.annoId+')"><img src="<%=ctx %>/images/icon/dels.png" width="22" height="25" /></span>'
                        		+ '</div>'
                        		+ '</a>'
                        		+ '<div  id="replyForm_'+parseInt((pageNo-1)*pageSize +i+1)+'" class="reply" style="display:none;">'
                        		+ '<input id="annoId_'+row.commentId+'" type="hidden" name="annoId"  value="'+row.annoId+'" />' 
                        		+ '<input id="repliedId_'+row.commentId+'" type="hidden" name="repliedId"  value="'+row.repliedId+'" />' 
                        		+ '<input id="repliedName_'+row.commentId+'" type="hidden" name="repliedName"  value="'+row.repliedName+'" />'
                        		...
                        		+ '<textarea placeholder="请输入回复内容..." class="reptext" id="comment_'+row.commentId+'" name="comment"></textarea>'
                        		+ '<hr class="link">'
...
<body style="background: #fff;">
	<div class="wrapper" style="overflow:visible;">
		<div class="header-public">
			<span style="cursor: pointer;" class="return" onclick="history.go(-1)"></span>评论详情
		</div>
		<div class="review" style="top:57px;  position:relative;">
			<div class="review-t">
				<h2 class="tiele">${businessAnno.annoTitle}</h2>
				<div class="newsinfo">
						公告范围：<span class="user">${businessAnno.annoScopeInfo}</span>   
						发布人：<span class="com">${businessAnno.publisherName}</span>   
						发布时间：<span class="com"><fmt:formatDate value="${businessAnno.publishTime }" pattern="yyyy-MM-dd HH:mm"/></span>
				</div>
				<p class="newsdeta">${businessAnno.brief}</p>
			...
			<input id="annoId" type="hidden" name="annoId"  value="${businessAnno.annoId}" />
			
			<div class="review-b">
				<h2 id="rowCount"  class="comnum">共计有${pager.rowCount}条评论</h2>
				<div id="commentDiv">
					<c:forEach items="${baseBean.list}" var="bean" varStatus="status">
						<div id="comment1_${bean.commentId }" class="cominfo">
							<a class="cominfobox" href="javascript:;"> 
								...
								<div class="opera">
									...
									<span class="del1" title="删除该评论" onclick="deleteComment('${bean.commentId}', '${bean.annoId}')">
							...
							<div  id="replyForm_${status.index+1}" class="reply" style="display:none;">
								<input id="annoId_${bean.commentId }" type="hidden" name="annoId"  value="${bean.annoId}" /> 
								<input id="repliedId_${bean.commentId }" type="hidden" name="repliedId"  value="${bean.repliedId}" /> 
								<input id="repliedName_${bean.commentId }" type="hidden" name="repliedName"  value="${bean.repliedName}" /> 
				            	...
				            	<textarea placeholder="请输入回复内容..." class="reptext" id="comment_${bean.commentId}" name="comment"></textarea>
				                <hr class="link">
