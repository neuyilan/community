	        	...
	        	<div class="scroll-box">
	                <ul id="oneul">
	                    ...
	                    <li id="fbType_" class="navlist"><a href="javascript:;"><span>所有分类</span><b class="donbut"><i></i></b></a>
	                    	...
	                    	<ul class="erjnav">
	                    		<li id="fbType_3"><a href="javascript:;">驿站建议</a></li>
	                        	<li id="fbType_4"><a href="javascript:;">快递投诉</a></li>
	                        </ul>
	                    </li>
	            ...
	            <c:forEach items="${baseBean.list }" var="feedback" varStatus="key">
	        		<div class="manbox"  <c:if test="${key.index % 3 == 0}" >style="margin-left:0;"</c:if> >
		            	<a class="nopotr2" href="javascript:;" style="height:296px;">
		                    ...
		                    <shiro:hasPermission name="station_feedback_view_detail">
	                    	<div class="operate operate2 blop" style="margin:0;" >
								<span class="see" style="position:relative; height:50px;" id="block5" title="查看建议投诉详情" onclick="window.location.href='${ctx}/business/businessFeedback/getStationFeedDetails.do?feedbackId=${feedback.feedbackId}&estateId=${feedback.estateId}&fberId=${feedback.fberId}<c:if test="${feedback.newsCount > 0 }">&isNew=1</c:if>'">
...
<script type="text/javascript">
    ...
    function jump(pageNo) {
    	...
    	$.ajax({
    		type: 'post',
            url: '<%=path %>/business/businessFeedback/getStationPageList.do',
            dataType: 'json',
            data: params,
            cache: false,
            success: function (data) {
                ...
                if(rows.length > 0) {
                	for(var i=0;i<rows.length;i++) {
                    	...
                    	<shiro:hasPermission name="station_feedback_view_detail">
                    	+ '<div class="operate operate2 blop" style="margin:0;" >'
                    	+'<span class="see" style="position:relative; height:50px;" title="查看建议投诉详情" id="block5" onclick="window.location.href=\'<%=ctx%>/business/businessFeedback/getStationFeedDetails.do?feedbackId='+row.feedbackId+'&estateId='+row.estateId+'&fberId='+row.fberId+(row.newsCount>0?'&isNew=1':'')+'\'">'
