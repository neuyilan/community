	        	...
	        	<div class="scroll-box">
	                <ul id="oneul">
	                    ...
	                    <li id="fbType_" class="navlist"><a href="javascript:;"><span>所有分类</span><b class="donbut"><i></i></b></a>
	                    	...
	                    	<ul class="erjnav">
	                    		<li id="fbType_2"><a href="javascript:;">使用反馈</a></li>
	                        </ul>
	                    </li>
	                    
	                    <%-- <c:if test="${orgType == 'station' || orgType == 'property' }">
		            	...
		            	<a class="nopotr2" href="javascript:;" style="height:296px;">
		                    <div class="info">
		                    	<p style="float:left;">建议投诉类型：
									...
									<span class="s-xw-sq" style="margin-left:60px;">
										所属范围：
										<c:choose>
										<c:when test="${fn:length(feedback.estateName) > 6}">
											<c:out value="${fn:substring(feedback.estateName, 0, 5)}..." />
										</c:when>
										<c:otherwise>
											<c:out value="${feedback.estateName}" />
										</c:otherwise>
									</c:choose>
									</span>
								</p>
		                    ...
		                    <shiro:hasPermission name="operation_feedback_view_detail">
	                    	<div class="operate operate2 blop" style="margin:0;" >
								<span class="see" style="position:relative; height:50px;" id="block5" title="查看建议投诉详情" onclick="window.location.href='${ctx}/business/businessFeedback/getOperationFeedDetails.do?feedbackId=${feedback.feedbackId}&estateId=${feedback.estateId}&fberId=${feedback.fberId}<c:if test="${feedback.newsCount > 0 }">&isNew=1</c:if>'">
...
<script type="text/javascript">
    ...
    function jump(pageNo) {
    	...
    	$.ajax({
    		type: 'post',
            url: '<%=path %>/business/businessFeedback/getOperationPageList.do',
            dataType: 'json',
            data: params,
            cache: false,
            success: function (data) {
                ...
                if(rows.length > 0) {
                	for(var i=0;i<rows.length;i++) {
                    	...
                    	+ '>'
                    	+ '<a class="nopotr2" href="javascript:;" style="height:296px;">'
                    	+ '<div class="info"><p style="float:left;">建议投诉类型：<span style="color: #3EAF0E;">'+fbType+'</span><span class="s-xw-sq" style="margin-left:60px;">所属范围：'+(row.estateName.length >6 ? row.estateName.substring(0, 5)+"..." : row.estateName)+'</p>'+fbType1+'</div>'
                    	...
                    	<shiro:hasPermission name="operation_feedback_view_detail">
                    	+ '<div class="operate operate2 blop" style="margin:0;" >'
                    	+'<span class="see" style="position:relative; height:50px;" title="查看建议投诉详情" id="block5" onclick="window.location.href=\'<%=ctx%>/business/businessFeedback/getOperationFeedDetails.do?feedbackId='+row.feedbackId+'&estateId='+row.estateId+'&fberId='+row.fberId+(row.newsCount>0?'&isNew=1':'')+'\'">'
