...
<html>
	...
	<body class="bagcolr">
	    <div class="wrapper wranews">	
	        <div class="newsrel">   
	        	<form id="attachform" action="update.do" method="post">
	            <div class="header-public"><span class="return" onclick="history.go(-1)"></span>编辑缴费通知</div>
	            <div class="cont-l">        
	               	<h2 class="relran">通知标题<label for="reportName" class="error success"></label></h2>
			       	<input class="iptnewtit" type="text"  name="reportName"  id="reportName"  value="${businessChargeAnno.reportName}" placeholder='请输入通知标题22字以内' />
		            ...
		            <div style="position:relative;">
		            	...
		            	<lable style="position: absolute; top: 10px; left: 160px;" id="reportExcel2">${businessChargeAnno.reportExcel}</lable>
		            </div>
		            <input type="hidden" name="reportExcel" id="reportExcel" value="${businessChargeAnno.reportExcel}"> <%-- 通知内容 --%>
	                <input type="hidden" name="users" id="users" value="${businessChargeAnno.users}">
	                <input type="hidden" name="reportId" id="reportId" value="${businessChargeAnno.reportId}">
	                
	                <div class="jftz">
	                	<div class="jf-title">
		                    <h3 class="jf-ttz"  style="cursor:default;"><img src="<%=ctx %>/images/icon/jfico.jpg"/>APP通知用户  <span><em id="user_app">${count2}</em>人</span></h3>
		                    <h3 class="jf-twtz"  style="cursor:default;"><img src="<%=ctx %>/images/icon/jfico.jpg"/>未注册无法通知  <span><em id="user">${count1}</em>人</span></h3>
		                  </div>
		                  <div class="jf-tz jf-ui">
		                    <ul id ="user_app_li">
		                    	<c:forEach items="${businessCharger2}" var="businessCharger2" varStatus="key">
		                    		<li>
				                        <div class="jf-in">
				                          <p>姓名：<span>${businessCharger2.name}</span><span>${businessCharger2.cellphone}</span></p>
				                          <p class="jf-top">小区：<span>${businessCharger2.estateName}</span></p>
				                          <p class="jf-top">楼栋：<span>${businessCharger2.building}号</span>单元：<span>${businessCharger2.unit}</span>门牌：<span>${businessCharger2.house}</span></p>
				                        </div>
				                        <div class="jf-con">
				                          <h4>通知内容：</h4>
				                          <p>${businessCharger2.content}</p>
				                        </div>
				                    </li>
		                    	</c:forEach>
		                    </ul>
		                  ...
		                  <div class="jf-wtz jf-ui">
		                    <ul id = "user_li">
		                    	<li>
			                        <h3>由于被通知人，没有安装APP,故无法进行"缴费通知"发送</h3>
			                    </li>
			                    <c:forEach items="${businessCharger1}" var="businessCharger1" varStatus="key">
		                    		<li>
				                    	<p>
				                          	<span class="jf-sml">姓名：<em>${businessCharger1.name}</em><em class="jf-pho">${businessCharger1.cellphone}</em></span>
				                          	<span>小区：<em>${businessCharger1.estateName}</em></span>
				                          	<span>楼栋：<em>${businessCharger1.building}号</em></span>
				                          	<span>单元：<em>${businessCharger1.unit}</em></span>
				                          	<span>门牌：<em>${businessCharger1.house}</em></span>
				                        </p>
				                    </li>
		                    	</c:forEach>
		                    </ul>
	        ...
	        <div class="sidebar4" style="height:100%;">
	          ...
	          <div id="wrapper-250">        
	            <ul class="accordion4">         
	              ...
	              <li class="sub-menu4">
	              	&nbsp;&nbsp;<input type="hidden" id="excelname" name="excelname" value="${businessChargeAnno.reportExcel}"/>
	              	<a style="color:#a6a6a6;" href="#">${businessChargeAnno.reportExcel}&nbsp;&nbsp;</a>
	              </li> 
	            </ul>          
	          </div>  
	          <div class="w-gg-btn">
