...
<html>
    ...
    <div class="wrapper wrapos">
        ...
        <div class="mainr">
	        	...
	        	<div class="scroll-box">
	                <ul id="oneul">
	                    ...
	                    <li id="annoType_" class="active navlist"><a href="javascript:;"><span>全部类型</span><b class="donbut"><i></i></b></a>
	                    	...
	                    	<ul class="erjnav">
	                    		<li id="annoType_0"><a href="javascript:;">通知类公告</a></li>
	                            <!-- <li id="annoType_1"><a href="javascript:;">信息传达类公告</a></li> -->
	                        </ul>
	                    </li>
	        ...
	        <div class="column">
	        	<!-- 发布公告开始 -->
	        	<shiro:hasPermission name="prop_anno_add_anno">
	        	<div class="manbox" style="margin-left:0;">
	            	<a class="nopotr" href="javascript:window.location.href='<%=ctx %>/business/businessAnno/propAdd.do';" style="cursor:pointer;">
	                    <div class="relnews">
	            ...
	            <c:forEach items="${baseBean.list }" var="anno" varStatus="key">
	        			...
	        			<shiro:lacksPermission name="prop_anno_add_anno">
		            	...
		            	<a class="nopotr" href="javascript:;">
		                    ...
		                    <div class="operate">
		                    	<shiro:hasPermission name="prop_anno_view_detail">
		                    		<span class="see"  title="查看公告详情"  id="block5" onclick="seeDetail('${anno.annoId}')"></span>
		                    	</shiro:hasPermission>
		                    	<c:if test="${anno.publishState != 0 }">
		                    		<shiro:hasPermission name="prop_anno_edit_anno">
		                    			<span class="edit" title="编辑公告" onclick="window.location.href='${ctx}/business/businessAnno/propModify.do?annoId=${anno.annoId}'"></span>
		                    		</shiro:hasPermission>
		                    	</c:if>
		                    	<shiro:hasPermission name="prop_anno_delete_anno">
...
<script type="text/javascript">
    ...
    function jump(pageNo) {
    	...
    	$.ajax({
            url: '<%=path %>/business/businessAnno/getPropPageList.do',
            type: 'post',
            dataType: 'json',
            data: params,
            cache: false,
            success: function (data) {
                ...
                if(rows.length > 0) {
                	for(var i=0;i<rows.length;i++) {
    		            ...
    		            <shiro:hasPermission name="prop_anno_edit_anno">
    		            +(row.publishState != 0?'<span class="edit" title="编辑公告" onclick="window.location.href=\'${ctx}/business/businessAnno/propModify.do?annoId='+row.annoId+'\'"></span>':'')
    		            </shiro:hasPermission>
    		            <shiro:hasPermission name="prop_anno_delete_anno">
	            ...
	            if(publishState == 0) {
	            	...
	            	if( data.annoType == 4){
		            	if(data.isImportant == "null" || data.isImportant == 0 ) {
		            		...
		            		var rightHtml = '';
		            		<shiro:hasPermission name="prop_anno_settop_anno">
		            			rightHtml += "<input id=\"qrbut5\" title=\"置顶该条公告\" type=\"submit\" value=\"设置为置顶\" onclick=\"setTopBtn("+annoId+");\"/>";
		            		</shiro:hasPermission>
		            		<shiro:hasPermission name="prop_anno_withdraw_anno">
	            	...
	            	publishState = '待审核';
	            	<shiro:hasPermission name="prop_anno_audit_anno">
    ...
    function editAnno(annoId) {
    	window.location.href = "propModify.do?annoId="+annoId;
    }
