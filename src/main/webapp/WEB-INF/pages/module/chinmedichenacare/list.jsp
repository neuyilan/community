...
<html>
        ...
        <div class="mainr">
            ...
            <div class="scroll">
	        	<div class="scroll-box">
	                <ul id="oneul">
	                	<li id="publishState_" class="active navlist"><a href="javascript:;"><span>全部养生</span><b class="donbut"><i></i></b></a>
	                    	<input type="hidden" name="publishState" id="publishState" value="${state}" />
	                    	<ul class="erjnav">
	        ...
	        <div class="column">
	        	<shiro:hasPermission name="care_publish">
	            <div class="manbox" style="margin-left:0;">
	            	<a class="nopotr" href="javascript:window.location.href='<%=ctx %>/business/businessChinmedichenacare/add.do';" style="cursor:pointer;">
	                    <div class="relnews">
                            <img src="<%=ctx %>/images/icon/relnews.png" style="width:100%;" />
                        </div>
	                	<span class="tittex">发布养生</span>
	                </a>
	            </div>
	            </shiro:hasPermission>
            	
	            <c:forEach items="${baseBean.list }" var="care" varStatus="key">
	        			<shiro:lacksPermission name="care_publish">
		            	...
		            	<a class="nopotr" href="javascript:;">
		                    <div class="info"></div>
		                    <time><fmt:formatDate value="${care.publishTime }" pattern="yyyy-MM-dd HH:mm"/></time>
		                    
		                    <hr class="link">
		                    <h2 class="title">${care.cmhcTitle }</h2>
		                    
		                    <div class="state">
									<c:if test="${care.publishState == 0 }"><span class="relsta cgreen">已发布</span></c:if>
									<c:if test="${care.publishState == 1 }"><span class="relsta cgray">未发布</span></c:if>
									<c:if test="${care.publishState == 2 }"><span class="relsta cyellow">待审核</span></c:if>
									<c:if test="${care.publishState == 3 }"><span class="relsta cred">未通过</span></c:if>
									<c:if test="${care.publishState == 4 }"><span class="relsta cblue">已通过</span></c:if>
								<div class="other-r">
									<c:choose>
									       <c:when test="${care.publishState == 0 && care.comments!=0}">
									            <i class="revlight"  title="评论量" style="cursor:pointer;" onclick="window.location.href='getNewsCommentList.do?cmhcId=${care.cmhcId}' ">${care.comments }</i>
									       </c:when>
									       <c:otherwise>
									              <i class="rev" title="评论量">${care.comments }</i>
									       </c:otherwise>
									</c:choose>
									<i class="look" title="浏览量">${care.visits }</i>
									<i class="help" id="block6" title="点赞量">${care.supports }</i>
								</div>
							</div>
		                    <div class="text">${care.brief}</div>
		                    <hr class="link">
		                    <div class="operate">
		                    	<shiro:hasPermission name="care_view_detail">
		                    	<span class="see s-xw-yfb" title="查看中医养生详情" onclick="checkNewsDetail('${care.cmhcId}');"></span>
		                    	</shiro:hasPermission>
		                    	<shiro:hasPermission name="care_edit">
		                    	<c:if test="${care.publishState != 0 }">
									<span class="edit"  title="编辑中医养生"  onclick="window.location.href='modify.do?cmhcId=${care.cmhcId}' "></span>
								</c:if>
								</shiro:hasPermission>
								<shiro:hasPermission name="care_delete">
								<span id="text1" class="del"  title="删除中医养生"  onclick="deleteNews('${care.cmhcId}')"></span>
		                    	</shiro:hasPermission>
		                    </div>
		                </a>
		            </div>
	...
	<div id="newsInfoLayer" class="busswi5 s-xw-bu">
        <div id="newsInfoBar" class="sidebar5 s-xw-si">
            ...
            <h2 class="tit5">中医养生内容<em>【<span id="showstate"></span>】</em></h2>
            <div id="wrapper-250">
                <ul class="accordion5">
    ...
    <div id="newsInfoLayer1" class="busswi5 s-xw-bu">
        <div id="newsInfoBar1" class="sidebar5 s-xw-si">
            ...
            <h2 class="tit5">中医养生内容<em>【<span id="showstate2"></span>】</em></h2>
            <div id="wrapper-250">
                <ul class="accordion5">
...
<script type="text/javascript">
	...
	function jump(pageNo) {
		...
		$.ajax({
			type: 'post',
	        url: '<%=path %>/business/businessChinmedichenacare/getPageList.do',
	        dataType: 'json',
	        data: params,
	        cache: false,
	        success: function (data) {
	            ...
	            if(rows.length > 0) {
	            	for(var i=0;i<rows.length;i++) {
	                	...
	                	var edit = '';
	                	if(row.publishState != 0){
	                		edit = '<span class="edit" title="编辑中医养生" onclick="window.location.href=\'modify.do?cmhcId='+row.cmhcId+'\' "></span>';
	                	}
	                	var htmlDom = ''
	              		+ '<div class="manbox"'
	    	        	+ styleStr
	                	...
	                	+ '<hr class="link">'
	                	+ '<h2 class="title">'+row.cmhcTitle+'</h2>'
	                	...
	                	+'onclick="'+((row.publishState ==0 && row.comments!=0)?'window.location.href=\'getNewsCommentList.do?cmhcId='+row.cmhcId+'\' ':'')+'">'+row.comments+'</i><i class="look" title="浏览量">'+row.visits+'</i><i class="help" id="block6" title="点赞量">'+row.supports+'</i></div></div>'
	                	+ '<div class="text">'+row.brief +'</div>'
	                	+ '<hr class="link">'
	                	+ '<div class="operate">'
	                	<shiro:hasPermission name="care_view_detail">
			            + '<span class="see s-xw-yfb"  title="查看中医养生详情" onclick="checkNewsDetail('+row.cmhcId+');"></span>'
			            </shiro:hasPermission>
			            <shiro:hasPermission name="care_edit">
			            + edit
			            </shiro:hasPermission>
			            <shiro:hasPermission name="care_delete">
			            + '<span id="text1" class="del"  title="删除中医养生" onclick="deleteNews('+row.cmhcId+')"></span>'
			            </shiro:hasPermission>
			            + '</div>'
	                	+ '</a>'		                
	                	+ '</div>';
	...
	//删除中医养生
	function deleteNews(cmhcId) {
		var bool = window.confirm("您确定要删除该条养生？");
	    if(bool) {
	        $.post("delete.do", {id : cmhcId}, function(data) {
	        	eval("data = "+data);
    			alert(data.message);
	...
	// 撤回发布该条中医养生
	function cancelNewsPublishState(cmhcId) {
		var bool = window.confirm("您确定撤回发布该条中医养生？");
	    if(bool) {
	        $.post("cancelNewsPublishState.do", {id : cmhcId}, function(data) {
	        	eval("data = "+data);
    			alert(data.message);
	            window.location.reload();
	        });
	...
	// 立即发布该条中医养生
	function cnChangeNewsState(cmhcId) {
		var bool = window.confirm("您确定立即发布该条中医养生？");
	    if(bool) {
	        $.post("updateNewsState.do", {id : cmhcId, auditInfo : ''}, function(data) {
	        	eval("data = "+data);
    			alert(data.message);
	            window.location.reload();
	        });
	...
	function auditPassNewsState(cmhcId) {
	    ...
	    if(bool) {
	        $.post("updateNewsStatePassNews.do", {id : cmhcId}, function(data) {
	        	eval("data = "+data);
    			alert(data.message);
	            window.location.reload();
	        });
	...
	// 拒绝该条中医养生
	function jjChangeNewsState(cmhcId) {
		$('#newsInfoLayer1').fadeIn("slow");
        
        $.post("getNewsDetail.do", {cmhcId : cmhcId}, function(data) {
        	eval("data = "+data);
            $('#title2').html(data.cmhcTitle);	//中医养生标题
        	...
        	$('#newsId2').html("<input type=\"hidden\" id=\"cmhcId\" value='"+data.cmhcId+"'/>");
            //发布状态
	...
	function updateNewsState() {
		if(($('#auditInfo').val()).trim() == '') {
			...
			var bool = window.confirm("您确定拒绝该条中医养生？");
		    if(bool) {
	...
	function checkNewsDetail(cmhcId) {
	  	$('#newsInfoLayer').fadeIn("slow");
        $.post("getNewsDetail.do", {cmhcId : cmhcId}, function(data) {
        	eval("data = "+data);
            $('#title').html(data.cmhcTitle);	//中医养生标题
            $('#content').html(data.cmhcContent);	//中医养生内容
            ...
            if(data.publishState == 0 ) {
            	...
            	$('#ding').html("");
            	/* if(data.isHot != 1) {
	            	// $('#ding').html("<input class=\"s-xw-btn1\" title=\"置顶该条中医养生\" type=\"button\" value=\"设置为置顶\" onclick=\"stickForm("+data.newsId+");\"/>");
	            	$('#ding').html("<input id=\"qrbut5\" title=\"置顶该条中医养生\" type=\"submit\" value=\"设置为置顶\" onclick=\"stickForm("+data.newsId+");\"/><input id=\"zsbut5\" title=\" 撤回发布该条中医养生\" type=\"button\" value=\"撤回发布\" onclick=\"cancelNewsPublishState("+data.newsId+");\"/>");
            	}else {
            		$('#ding').html("<input class=\"s-xw-btn1\" title=\" 撤回发布该条中医养生\" type=\"button\" value=\"撤回发布\" onclick=\"cancelNewsPublishState("+data.newsId+");\"/>");
            	} */
            	<shiro:hasPermission name="care_withdrawn">
            	$('#ding').html("<input class=\"s-xw-btn1\" title=\" 撤回发布该条中医养生\" type=\"button\" value=\"撤回发布\" onclick=\"cancelNewsPublishState("+data.cmhcId+");\"/>");
            	</shiro:hasPermission>
            	$("#auditInfo1").css('display','none'); 
		         $("#hr1").css('display','none'); 
            } else if(data.publishState == 1 ) {
            	$('#showstate').html("未发布");
            	// $('#ding').html("<input class=\"s-xw-btn2\" title=\"置顶该条中医养生\" type=\"button\" value=\"设置为置顶\" />");
            	$('#ding').html("");
            	<shiro:hasPermission name="care_edit">
            	$('#ding').html("<input class=\"s-xw-btn1\" title=\"编辑中医养生\" type=\"button\" value=\"编辑\" onclick=\"window.location.href='modify.do?cmhcId="+data.cmhcId+"' \"/>");
            	</shiro:hasPermission>
            	$("#auditInfo1").css('display','none'); 
		        $("#hr1").css('display','none'); 
            } else if(data.publishState == 2 ) {
            	$('#showstate').html("待审核");
            	$('#ding').html("");
            	<shiro:hasPermission name="care_audit">
            	$('#ding').html("<input id=\"qrbut5\" title=\"审核通过该条中医养生\" type=\"submit\" value=\"审核通过\" onclick=\"auditPassNewsState("+data.cmhcId+");\"/><input id=\"zsbut5\" title=\"拒绝该条中医养生\" type=\"button\" value=\"拒绝\" onclick=\"jjChangeNewsState("+data.cmhcId+");\"/>");
            	</shiro:hasPermission>
            	$("#auditInfo1").css('display','none'); 
		        $("#hr1").css('display','none'); 
            }else if(data.publishState == 3 ) {
            	...
            	$('#ding').html("<input class=\"s-xw-btn1\" title=\"立即发布该条中医养生\" type=\"submit\" value=\"立即发布\" onclick=\"cnChangeNewsState("+data.cmhcId+");\"/>");
            	$("#auditInfo1").css('display','none'); 
		        $("#hr1").css('display','none'); 
		        </shiro:hasPermission>
            }
