...
<script type="text/javascript">
        ...
        //删除评论
        function deleteComment(commentId, cmhcId) {
        	var bool = window.confirm("您确定要删除该条评论吗？");
    	    if(bool) {
    	    	$.post('${ctx}/business/businessChinmedichenacareComment/delete.do', {id :commentId, cmhcId:cmhcId}, function(data) {
    	    		eval("data = "+data);
        			alert(data.message);
        ...
        function saveComment(commentId) {
        	...
        	if(content == ''){
            	...
            	$.post('${ctx}/business/businessChinmedichenacareComment/replySave.do', {
            		cmhcId: $('#cmhcId_'+commentId).val(),
      ...
      //获取多参数
    	function getParams() {
    		...
    		var params = {cmhcId: '${businessChinmedichenacare.cmhcId}', keyWord: keyWord};
    		return params;
    	}
        ...
        function jump(pageNo, pageSize) {
        	...
        	$.ajax({
        		type: 'post',
                url: '<%=path %>/business/businessChinmedichenacareComment/getPageList_manage.do',
                dataType: 'json',
                data: params,
                cache: false,
                success: function (data) {
                    ...
                    if(rows.length > 0) {
                    	...
                    	for(var i=0;i<rows.length;i++) {
                        	...
                        	var htmlDom = ''
                        		...
                        		+ '<span class="del1"title="删除该评论" onclick="deleteComment('+row.commentId+', '+row.cmhcId+')"><img src="<%=ctx %>/images/icon/dels.png" width="22" height="25" /></span>'
                        		+ '</div>'
                        		+ '</a>'
                        		+ '<div  id="replyForm_'+parseInt((pageNo-1)*pageSize +i+1)+'" class="reply" style="display:none;">'
                        		+ '<input id="cmhcId_'+row.commentId+'" type="hidden" name="cmhcId"  value="'+row.cmhcId+'" />' 
...
<body style="background: #fff;">
	<div class="wrapper" style="overflow:visible;">
		<div class="header-public">
			...
			<%-- <span class="return" onclick="window.location.href = '<%=ctx %>/business/businessChinmedichenacare/list.do?id=${id}';" style="cursor:pointer;"></span>评论详情 --%>
		</div>
		<div class="review" style="top:57px;  position:relative;">
			<div class="review-t">
				<h2 class="tiele">${businessChinmedichenacare.cmhcTitle}</h2>
				<div class="newsinfo">
						发布时间：
						<span class="com"><time>
							<fmt:formatDate value="${businessChinmedichenacare.publishTime }" pattern="yyyy-MM-dd HH:mm" />
						</time></span>
				</div>
				<p class="newsdeta">${businessChinmedichenacare.cmhcContent}</p>
						...
						<div id="comment_${bean.commentId }" class="cominfo">
							<a class="cominfobox" href="javascript:;"> 
								...
								<div class="opera">
									...
									<span class="del1" onclick="deleteComment('${bean.commentId}', '${bean.cmhcId}')">
							...
							<div  id="replyForm_${status.index+1}" class="reply" style="display:none;">
								<input id="cmhcId_${bean.commentId }" type="hidden" name="cmhcId"  value="${bean.cmhcId}" /> 
