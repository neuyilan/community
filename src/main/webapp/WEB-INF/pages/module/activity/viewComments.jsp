...
<script type="text/javascript">
        ...
        //删除评论
        function deleteComment(commentId, actId) {
        	var bool = window.confirm("您确定要删除该条评论吗？");
    	    if(bool) {
    	    	$.post('${ctx}/business/businessActivityComment/delete.do', {id :commentId, actId:actId }, function(data) {
    	    		eval("data = "+data);
        			alert(data.message);
        ...
        function saveComment(commentId) {
        	...
        	if(content == ''){
            	...
            	$.post('<%=ctx %>/business/businessActivityComment/replySave.do', {
            		actId: $('#actId_'+commentId).val(),
      ...
      //获取多参数
        ...
        function jump(pageNo, pageSize) {
        	...
        	$.ajax({
                url: '<%=path %>/business/businessActivityComment/getPageList.do',
                dataType: 'json',
                data: params,
                cache: false,
                type: 'post',
                    ...
                    if(rows.length > 0) {
                    	...
                    	for(var i=0;i<rows.length;i++) {
                        	...
                        	var htmlDom = ''
                        		...
                        		+ '<span class="del1" title="删除该评论" onclick="deleteComment('+row.commentId+', '+row.actId+')"><img src="<%=ctx %>/images/icon/dels.png" width="22" height="25" /></span>'
                        		+ '</div>'
                        		+ '</a>'
                        		+ '<div  id="replyForm_'+parseInt((pageNo-1)*pageSize +i+1)+'" class="reply" style="display:none;">'
                        		+ '<input id="actId_'+row.commentId+'" type="hidden" name="actId"  value="'+row.actId+'" />' 
	...
	<div class="wrapper" style="overflow:visible;">
		...
		<div class="review" style="top:57px;  position:relative;">
			<div class="review-t">
				...
				<div class="newsinfo">
					活动范围：<span class="com">${businessActivity.actScope}</span> 活动时间：<span class="com">
					<time>
							${businessActivity.publishDate }&nbsp;${businessActivity.publishTime }
						</time></span>
				</div>
				<p class="newsdeta">${businessActivity.actContent}</p>
					...
					<c:forEach items="${baseBean.list}" var="bean" varStatus="status">
						<div id="comment_${bean.commentId }" class="cominfo">
							<a class="cominfobox" href="javascript:;"> 
								...
								<div class="textinfo">
									<span class="aut">${empty bean.commentor?"匿名": bean.commentor}
								...
								<div class="opera">
									...
									<span class="del1" title="删除该评论" onclick="deleteComment('${bean.commentId}', '${bean.actId}')">
										<img src="${ctx}/images/icon/dels.png" width="22" height="25" />
									</span>
								</div>
							</a>
							<div  id="replyForm_${status.index+1}" class="reply" style="display:none;">
								<input id="actId_${bean.commentId }" type="hidden" name="actId"  value="${bean.actId}" /> 
