...
<html>
    ...
    <div class="wrapper wrapos">
        ...
        <div class="mainr">
	        ...
	        <div class="column">
	            <shiro:hasPermission name="focus_ad_publish">
		        	<div class="manbox"  style="margin-left:0;">
		            	<a class="nopotr2 s-baoliao-no" href="javascript:window.location.href='<%=ctx %>/business/businessFocusAd/add.do';" style="cursor:pointer; height:326px;">
		                    <div class="relnews">
	            ...
	            <c:forEach items="${baseBean.list }" var="focusAd" varStatus="key">
	            		
	            		<shiro:lacksPermission name="focus_ad_publish">
		            	...
		            	<a class="nopotr2 s-baoliao-no" href="javascript:;" style="height:326px;"> 
		                    <div class="info">
		                    	<p style="float:left;">范围：<span style="color: #3EAF0E;">
			                    	<c:choose>
										<c:when test="${fn:length(focusAd.focusAdScope) > 27}">
											<c:out value="${fn:substring(focusAd.focusAdScope, 0, 26)}......" />
										</c:when>
										<c:otherwise>
											<c:out value="${focusAd.focusAdScope}" />
										</c:otherwise>
									</c:choose>
		                    	</span></p>
		                    </div>
		                    <time><fmt:formatDate value="${focusAd.selectTime }" pattern="yyyy-MM-dd HH:mm"/></time>
		                    <hr class="link">
		                    <h2 class="s-jdt-title title">${focusAd.title }</h2>
		                    <div class="state">
								<c:if test="${focusAd.state == 0 }"><span class="relsta cgreen">已发布</span></c:if>
								<c:if test="${focusAd.state == 1 }"><span class="relsta cgray">未发布</span></c:if>
								<c:if test="${focusAd.state == 2 }"><span class="relsta cyellow">待审核</span></c:if>
								<c:if test="${focusAd.state == 3 }"><span class="relsta cred">未通过</span></c:if>
								<c:if test="${focusAd.state == 4 }"><span class="relsta cblue">已通过</span></c:if>
								<div class="other-r">
									<i class="look" title="浏览量">${focusAd.visits }</i>
								</div>
							</div>
		                	<img class="s-jdt-img" src="<%=ctx %>${focusAd.picUrl}"/>
		                    <hr class="link">
                    		<div class="operate">
	        ...
	        <div id="wrapper-250">
	            <ul class="accordion5">
	                ...
	                <ul class="sub-menu5 s-xw-xx">
		              	<li><span>范围：<lable id="focusAdScope1"></lable></span></li>
        ...
        <div id="focusAdInfoBar1" class="sidebar5 s-xw-si">
            ...
            <div id="wrapper-250">
                <ul class="accordion5">
                    ...
                    <div class="link5"></div>
                    <ul class="sub-menu5 s-xw-xx">
		                <li><span>类型：<lable id="focusAdScope2"></lable></span></li>
		                <li><span class="xxl">选用人：<lable id="selectorName2"></lable></span><span class="xxr">选用时间：<lable id="selectTime2"></lable></span></li>
		                <div class="link5"></div>
		                <li><div class="s-xw-con"><lable id="focusAdId2"></lable><textarea class="iptnewtit" id="auditInfo" name="auditInfo" style="width:435px;height:200px; font-size:14px; padding-top:5px; padding-left:5px;" placeholder="请输入拒绝原因，发送至提交人" ></textarea></div></li>
	              	</ul>
                </ul>
            </div>
            <div class="s-xw-zd">
	      		<div class="link6"></div>
	          	<div class="submtpres">
	                  <input id="qrbut7" type="button"  value="确定" onclick="updateFocusAdState()"/>
	                  <input id="zsbut7" type="button"  value="取消" onclick="$('#focusAdInfoLayer1').fadeOut('slow');"/>
	          	</div>
    		</div>
        </div>
    </div>
...
<script type="text/javascript">
	...
	//编辑
    function editFocusAdId(focusAdId) {
    	window.location.href = "modify.do?focusAdId="+focusAdId;
    }
	
    function checkFocusAdDetail(focusAdId) {
	...
	function jjChangeFocusAdState(focusAdId) {
        ...
        $.post("getFocusAdDetail.do", {focusAdId : focusAdId}, function(data) {
            ...
            $('#focusAdScope2').html((data.focusAdScope).replaceAll(",", " | "));		//选用范围
        	$('#focusAdId2').html("<input type=\"hidden\" id=\"focusAdId\" value='"+data.focusAdId+"'/>");
            //发布状态
            if(data.state == 0 ) {
            	$('#showstate2').html("已发布");
            } else if(data.state == 1 ) {
    ...
    function jump(pageNo) {
    	...
    	$.ajax({
    		type: 'post',
            url: '<%=path %>/business/businessFocusAd/getPageList.do',
            dataType: 'json',
            data: params,
            cache: false,
            success: function (data) {
                ...
                if(rows.length > 0) {
                	for(var i=0;i<rows.length;i++) {
                    	...
                    	+ '<a class="nopotr2 s-baoliao-no" href="javascript:;" style="height:326px;"> '
                    	+ '<div class="info"><p style="float:left;">范围：<span style="color: #3EAF0E;">'+(row.focusAdScope.length >27 ? row.focusAdScope.substring(0, 26)+"......" : row.focusAdScope)+'</span></p> </div>'
