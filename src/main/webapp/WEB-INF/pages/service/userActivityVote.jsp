			        			...
			        			<c:if test="${voteType == 2}">
		                            <div class="tp-table">
		                                ...
		                                <div class="tp-rediv tp-tc tp-re">
		                                    <p><i class="reso tp-reic" style="width:${businessActivityVoteOptions.percentage};background-color:<c:choose><c:when test="${status.index%5 == 1}">#a5ce45</c:when><c:when test="${status.index%5 == 2}">#e33e68</c:when><c:when test="${status.index%5 == 3}">#feb04f</c:when><c:when test="${status.index%5 == 4}">#25a4de</c:when><c:when test="${status.index%5 == 5}">#bf74d9</c:when></c:choose>"></i></p>
		                                    <p>票数：<span>${businessActivityVoteOptions.count}</span> (<span>${businessActivityVoteOptions.percentage}</span>)</p>
		                                </div> 
		                            </div>
			        			</c:if>
