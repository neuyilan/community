...
<div id="picUploadLayer1" class="busswi5 s-xw-bu">
	<div id="picUploadBar1" class="sidebar5 s-xw-si s-xw-show">
		<a id="close5" title="关闭" href="javascript:;" onclick="$('#picUploadLayer1').fadeOut('slow');"></a>
...
function uploadInit1(folderName, fieldName, imgType, flag, type) {
	        ...
	        success: function (data) {
	            ...
	            if(rows.length > 0) {
	            	...
	            	for(var i=0;i<rows.length;i++) {
	                	...
	                	+'<div class="delinf">'
	                	+'<p><img src="<%=ctx%>'+row.imgPath+'"  width="130" height="130" style="float:left; padding-top:20px; padding-right:20px;"></p>'
	                	+'<p><a>'+row.imgName+'</a></p>'
	                	+'</div>'
	                	+'</li>';
	...
	if(arr.length > 0){
		 ...
		 for(var i=0; i<arr.length; i++) {
			 $('#imageShow1').append(
		          ...
		          '</a>'+
		          '<span class="delet4" id="btnDeleteImg" onclick="delImg1(\''+uploadField1+'\', \''+arr[i].substring(arr[i].lastIndexOf('/')+1,arr[i].lastIndexOf('.'))+'\',\''+arr[i].substring(arr[i].lastIndexOf('/image'))+'\')"></span>'+
		          '</li> '
		     );
		 }
	}
	...
	$('#imageShow1').empty();
	var uploadField1 = $('#uploadField1').val();
	if(uploadField1 != '' && uploadField1 == fieldName) {
	    $('#'+uploadField1+'Btn').attr('src', '<%=ctx %>/images/icon/add.jpg');
	}
